...
PORT_UP=1
while [[ ! -e "${CONFIG_FILE}" || ! -d "/var/run/s6/services/plexpy" || $(s6-svstat -u "/var/run/s6/services/plexpy") != "true" || $PORT_UP -ne 0 ]]; do
	sleep 1s
...
if [[ -d "/var/run/s6/services/plexpy" && $(s6-svstat -u "/var/run/s6/services/plexpy") == "true" && "${HASH_START}" != "${HASH_END}" ]]; then
	s6-svc -h "/var/run/s6/services/plexpy"
	s6-svwait -u "/var/run/s6/services/plexpy"
fi
